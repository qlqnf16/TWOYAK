...
function Login(props) {
  ...
  const signinHandler = () => {
    ...
    axios
      ...
      .then(response => {
        ...
        localStorage.setItem('id', jwt_decode(payload).user.id)
        localStorage.setItem('email', jwt_decode(payload).user.email)
        localStorage.setItem('user_name', jwt_decode(payload).user.user_name)
        localStorage.setItem('user_info_id', jwt_decode(payload).user.user_info_id)
        props.history.push('/medicine')
      })
  }
  ...
  return (
    <div>
      ...
      <button onClick={() => signinHandler()}>그냥 로그인</button>
